...
trigger: "User asks to write, review, or refactor NestJS code: modules, DI, security, validation, performance."
license: MIT
metadata:
  author: Kadajett
  version: "1.2.0"
...
## Biome Compatibility (critical)

**Biome's `useImportType` auto-fix breaks NestJS DI** — it rewrites injected service imports to `import type`, erasing `design:paramtypes` metadata → runtime crash "can't resolve dependencies". Biome officially recommends disabling this rule for NestJS (see docs caveat "TypeScript experimental decorators").

Before running `biome check --write` on NestJS code:
1. Add `javascript.parser.unsafeParameterDecoratorsEnabled: true` (else parse errors on `@Body()`/`@Param()`)
2. Override `useImportType: off` for backend paths
3. Recommended: `useExplicitReturnType: warn` (nursery)

**See:** `references/biome-compatibility.md` — full verified config, official issue links (#4514, #2038, #10495, #5305), and the shared-package compile trap (`ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING`).
